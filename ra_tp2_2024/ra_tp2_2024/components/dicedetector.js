...
import SURF from '../deps/surf/surf.js';
import POS from '../deps/pose/posit_pnp.js';
import jsfeat from '../deps/jsfeat/jsfeat.js';
...
DiceDetector = ARCS.Component.create(
    function(initData) {
        ...
        var marker = {};
        var i, currentId;
        var faceCanvas, DiceFace, poseEstimator, videoCanvas;
        ...
        if (initData.videoCanvas !== undefined) {
            videoCanvas = document.getElementById(initData.videoCanvas);
        }
        
        faceCanvas = document.getElementById(initData.faceCanvas);
        poseEstimator = new POS.PnPSolver(); //new POS.Posit(600);
        ...
        */        
        var DiceFace = function(descr) {
            ...
            this.debugImage = function() {
                // TODO #4
                // By using recoverImageData or the object faceCanvas, 
...
*****************************************************************************/
        ...
        console.log("/**** Descripteurs détectés ****/\n", JSON.stringify(diceFaces.map(
            function(item) {
                let res = {};
                res[item.imageId] = item.keyPoints.length; 
                return res;
            }
        )));
        ...
        this.detect = function(image) {
            ...
            // variable keyPoints. Documentation of global SURF object 
            // will give hints about using surfDetDes(...)
            // then represent this first batch of detected points 
            // using SURF.drawIPoints
            ...
            for (i =0; i< 6; i++) {
                if (keyPoints.length > 0) {                            
                    ...
                    if (matches.length >= 6) {
                        ...
                        // TODO #9
                        // look at jsfeat.motion_estimator.ransac to determine how to be able to generate
                        // a mask determining if a keypoint is an inlier or outlier.
                        ...
                        for (j=0; j < matches.length; j++) {
                            ...
                            //
                            // Do not forget to adjust behaviour according to mask computed
                            // with ransac (after doing TODO #9)
            ...
            if( points2D.length >= 4) {
                ...
                // (img). You will use drawIPoints(...) defined in global object SURF
                // do not forget to deactivate your previous drawings of keyPoints
                // on this image
                ...
                // However, do not forget the actual pose estimator 
                // is of type PnPSolver which behaves like Posit.
        ...
        this.setIntrinsics = function(params) {
            poseEstimator.setIntrinsics(params);            
        };
...
export default { DiceDetector : DiceDetector };
